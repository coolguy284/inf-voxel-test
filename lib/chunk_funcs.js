...
function chunkCoordsToID(dimension, cx, cy, cz) {
  return dimension + ':' + cx + ',' + cy + ',' + cz;
}

function chunkIDToCoords(chunkID) {
  let chunkIDSplit = chunkID.split(':');
  let [ dimension, chunkPosID ] = [chunkIDSplit.slice(0, chunkIDSplit.length - 1).join(':'), chunkIDSplit[chunkIDSplit.length - 1]];
  let chunkPosStrings = chunkPosID.split(',');
  return [
    dimension,
