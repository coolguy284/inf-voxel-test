...
class ChunkManager {
  ...
  #validateCoords(dimension, x, y, z) {
    if (typeof dimension != 'string') {
      throw new Error('dimension not string');
    }
    
    if (!worldData.dimensions.has(dimension)) {
      throw new Error('dimension nonexistent');
    }
  ...
  getChunkAt(dimension, cx, cy, cz) {
    this.#validateCoords(dimension, cx, cy, cz);
    ...
    if (currentChunk == null) {
      ...
      this.setChunkAt(dimension, cx, cy, cz, chunk);
  ...
  setChunkAt(dimension, cx, cy, cz, chunk) {
    this.#validateCoords(dimension, cx, cy, cz);
    
    this.#chunks.set(chunkCoordsToID(cx, cy, cz), chunk);
  }
  
  getBlockAt(dimension, x, y, z) {
    this.#validateCoords(dimension, x, y, z);
    
    let [ cx, cy, cz, bx, by, bz ] = blockCoordsToChunkCoords(x, y, z);
    
    let chunk = this.getChunkAt(dimension, cx, cy, cz);
    let block = chunk.getBlockAt(bx, by, bz);
    
    return block;
  }
  
  setBlockAt(dimension, x, y, z, blockName) {
    this.#validateCoords(dimension, x, y, z);
  ...
  getChunkActiveMesh(dimension, cx, cy, cz) {
    this.#validateCoords(dimension, cx, cy, cz);
    
    return this.#chunksWithActiveMesh.has(chunkCoordsToID(dimension, cx, cy, cz));
  }
  
  setChunkActiveMesh(dimension, cx, cy, cz) {
    this.#validateCoords(dimension, cx, cy, cz);
    
    return this.#chunksWithActiveMesh.add(chunkCoordsToID(dimension, cx, cy, cz));
  }
  
  clearChunkActiveMesh(dimension, cx, cy, cz) {
    this.#validateCoords(dimension, cx, cy, cz);
    
    return this.#chunksWithActiveMesh.delete(chunkCoordsToID(dimension, cx, cy, cz));
  }
